...
### 22.打印最长连续递增子序列
### 23.没有优先级的"+","-","x"
### 24.在不使用循环的条件下，如何计算出一个数其2的n次方的最大数字（这个比较绕，例如17-->16，31-- >16）
### 25.未排序数组中累加和为给定值的最长子数组的长度
### 26.N的阶乘二进制表示的最低位1的位置
### 27.输入一个正整数N，输出大于N且最接近这个数的素数
### 28.字符串转换float
### 29.字符串数组的字符串是否都能首尾相接
### 30.递归求和
### 31.递归算法判断一个数组是否是递增数组
### 32.二分查找的递归与非递归实现
### 33.正整数序列Q中的每个元素都至少能被正整数a和b中的一个整除，现给定a和b，如何计算出Q中的前N项？
### 34.如何判断一个整数 x 是否可以表示成 n（n >= 2）个连续正整数的和?
### 35.给一个由n-1个数组成的未经排序的数列，其元素都是1—n中的不同的整数。找出缺失的整数？
### 36.求两个数组的交集
### 37.如何判断一个数组中的数值是否连续相邻
### 38.找出数组中重复数字最多的数
### 39.将数组的后m个数移动为前m个数
### 40.找出数组中出现奇数次的元素：有一个整数数组arr，其中的元素只有一个元素出现奇数次，请找出这个元素。
### 41.数组a[N]，1-N-1这（N-1）个数存放在a[N]中，其中某个数重复一次，找出这个数。时间复杂度不超过
### 42.一个序列先增后减，求峰值
### 43.递归求数组的和
### 44.输入一个字符串，输出该字符串中字符的所有组合。举个例子，如果输入abc，它的组合有a、b、c、ab、ac、bc、abc
### 45.在两个数组中寻找两个数的和等于指定的数值
### 46.计算给定的日期是一年中的第多少天
### 47.求A的B次的后三位
### 48.最长公共子序列（不连续）时间复杂度O(m*n)
### 49.最长公共子字符串：类似最长子序列，只是公共子字符串要求必须是连续的
### 50.M进制与N进制的转换
### 51.等概率产生0和1
### 52.求一个序列的平衡点
